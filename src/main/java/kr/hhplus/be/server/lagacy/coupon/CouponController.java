package kr.hhplus.be.server.lagacy.coupon;
...
import kr.hhplus.be.server.lagacy.coupon.dto.CouponIssueRequest;
import kr.hhplus.be.server.lagacy.coupon.dto.CouponListResponse;
import kr.hhplus.be.server.lagacy.coupon.dto.CouponResponse;
import kr.hhplus.be.server.lagacy.coupon.docs.CouponSwaggerDocs;
...
public class CouponController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "쿠폰 발급 성공",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "발급 성공", value = CouponSwaggerDocs.ISSUE_SUCCESS))),
            @ApiResponse(responseCode = "400", description = "유효하지 않은 쿠폰 ID",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "잘못된 쿠폰 ID", value = CouponSwaggerDocs.INVALID_COUPON_ID))),
            @ApiResponse(responseCode = "404", description = "쿠폰 없음",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "쿠폰 없음", value = CouponSwaggerDocs.COUPON_NOT_FOUND))),
            @ApiResponse(responseCode = "409", description = "잔여 수량 없음 / 중복 발급",
                    content = @Content(mediaType = "application/json",
                            examples = {
                                    @ExampleObject(name = "잔여 수량 없음", value = CouponSwaggerDocs.NO_MORE_COUPONS),
                                    @ExampleObject(name = "이미 발급받음", value = CouponSwaggerDocs.ALREADY_ISSUED)
                            }
                    )
            )
    })
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "조회 성공 예시", value = CouponSwaggerDocs.COUPON_LIST_SUCCESS))),
            @ApiResponse(responseCode = "400", description = "잘못된 사용자 ID",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "잘못된 userId", value = CouponSwaggerDocs.INVALID_USER_ID)))
    })
    @GetMapping
