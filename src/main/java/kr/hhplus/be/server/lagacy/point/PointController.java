package kr.hhplus.be.server.lagacy.point;
...
import kr.hhplus.be.server.lagacy.point.docs.PointSwaggerDocs;
import kr.hhplus.be.server.lagacy.point.dto.PointChargeRequest;
import kr.hhplus.be.server.lagacy.point.dto.PointChargeResponse;
import kr.hhplus.be.server.lagacy.point.dto.PointUseRequest;
    ...
    @ApiResponses({
            @ApiResponse(
                    ...
                    content = @Content(
                            ...
                            examples = @ExampleObject(name = "충전 성공 예시", value = PointSwaggerDocs.CHARGE_SUCCESS)
                    )
            ),
            @ApiResponse(
                    responseCode = "400",
                    ...
                    content = @Content(
                            mediaType = "application/json",
                            examples = @ExampleObject(name = "잘못된 사용자 ID", value = PointSwaggerDocs.INVALID_USER_ID)
                    )
            ),
            @ApiResponse(
                    responseCode = "409",
                    ...
                    content = @Content(
                            mediaType = "application/json",
                            examples = @ExampleObject(name = "충전 금액 초과", value = PointSwaggerDocs.CHARGE_OVER_LIMIT)
                    )
            ),
