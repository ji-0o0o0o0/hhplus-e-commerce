...
import kr.hhplus.be.server.product.dto.BestProductListResponse;
import kr.hhplus.be.server.product.dto.BestProductResponse;
import kr.hhplus.be.server.product.dto.ProductListResponse;
...
import static kr.hhplus.be.server.product.docs.ProductSwaggerDocs.*;
...
public class ProductController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "상품 목록", value = PRODUCT_LIST))),
            @ApiResponse(responseCode = "500", description = "서버 내부 오류",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "서버 오류", value = INTERNAL_ERROR)))
    })
    @GetMapping
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "인기 상품", value = BEST_PRODUCTS))),
            @ApiResponse(responseCode = "500", description = "서버 내부 오류",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "서버 오류", value = INTERNAL_ERROR)))
    })
